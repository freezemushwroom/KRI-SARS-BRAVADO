...
# UPDATED: Reduced rear extension and roll parameters to lower rear profile
mundur = -10     # Reduced from -20 to bring rear legs closer and lower
roll = -12       # Reduced from -18 to lower calculated rear IK standing height
flag = 0
forward = 4
mepet = 0
flag_stop_tangga = 1
...
try:
    while True:
        ...
        # PHASE 4: REAR LEG RECOVERY (UPDATED REDUCED LIFT HEIGHT: 55 Femur / 92 Tibia)
        # femur tibia 3 angkat
        kit1.servo[1].angle = max(0, min(((coxa_femur_belakang_berdiri+35) + 55), 180)) #femur3 (reduced from +60)
        kit1.servo[0].angle = max(0, min((180 - (femur_tibia_belakang_berdiri-45) + 92), 180)) #tibia3 (reduced from +100)
        wait(0.5)
        ...
        kit2.servo[14].angle = max(0, min((180 - (coxa_femur_belakang_berdiri+35) - 55), 180)) #femur4 (reduced from -60)
        kit2.servo[15].angle = max(0, min(((femur_tibia_belakang_berdiri-45) - 92), 180)) #tibia4 (reduced from -100)
        wait(0.5)
