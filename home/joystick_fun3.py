...
def register_joystick():
    """
    Refresh pygame joystick system and create a new joystick object.
    This is useful if the Logitech controller was turned on late
    or was not fully registered at startup.
    """
    pygame.event.pump()
    ...
    print("Connected to:", js.get_name())
...
def wait_for_joystick():
    """
    Wait until joystick is connected and fully registered.
    For your case, Logitech should reach button index 11,
    so we wait until get_numbuttons() >= 12.
    """
    global start_time

    while True:
        js = register_joystick()

        if js is not None:
            if js.get_numbuttons() >= 12:
                print("Joystick fully registered.")
                return js
            else:
                print(f"Joystick not fully registered yet. Buttons: {js.get_numbuttons()} / 12")

        if time.time() - start_time >= timeout:
            print("Joystick timeout, still waiting...")

            # Safety condition
            stop_event.set()
            GPIO.output(LED_PIN, GPIO.HIGH)

            # Reset timeout timer, but do not exit program
            start_time = time.time()
...
try:
    while True:
        ...
        try:
            ...
            print("Joystick disconnected or invalid. Re-registering...")
        ...
        # ---- IF BUTTON RANGE IS INCOMPLETE, RE-REGISTER ----
        if num_buttons < 12:
            print(f"Joystick button range incomplete: {num_buttons} / 12")
            print("Trying to re-register joystick...")
        ...
        # ---- HATS ----
        hats = joystick.get_hat(0) if num_hats > 0 else (0, 0)
